...
markdown.runTests(testCase('Bold conversion', () => {
    const md = '**STRONG!**';
...
templateTests.runTests(testCase('Simple lines should appear the same', (env) => {
    ...
    env.engine.includes.set('intro.html', '% for (let i of [0, 1]) {\n' +
        '${i} ${words[i]}\n% }');
    const template = `% include intro.html
        <p> Content </p>`;
...
postsTests.runTests(testCase('Parse post file name', (sample) => {
